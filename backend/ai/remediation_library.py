...
AZURE_NETWORK_WATCHER = RemediationTemplate(
    ...
    cli="""az network watcher configure \
...
AZURE_NETWORK_FLOW_LOG = RemediationTemplate(
    ...
    cli="""az network watcher flow-log create \
  --resource-group "{rg}" \
...
AZURE_NETWORK_FLOW_LOG_90 = RemediationTemplate(
    ...
    cli="""az network watcher flow-log update \
  --resource-group NetworkWatcherRG \
  --name "{resource}" \
  --retention 90""",
    terraform="""resource "azurerm_network_watcher_flow_log" "main" {
  # (Attach to existing NSG flow log resource)
  retention_policy {
    enabled = true
    days    = 90
  }
...
AZURE_NETWORK_DDOS = RemediationTemplate(
    ...
    cli="""# Create DDoS plan if needed
...
AZURE_NETWORK_SUBNET_NSG = RemediationTemplate(
    ...
    cli="""az network vnet subnet update \
  --resource-group "{rg}" \
  --vnet-name your-vnet \
  --name "{resource}" \
...
AZURE_STORAGE_VERSIONING = RemediationTemplate(
    ...
    cli="""az storage account blob-service-properties update \
...
AZURE_STORAGE_INFRA_ENCRYPTION = RemediationTemplate(
    ...
    cli="""# NOTE: Infrastructure encryption must be set at creation time.
...
AZURE_ENTRA_MFA_PRIVILEGED = RemediationTemplate(
    ...
    cli="""# Enable Conditional Access MFA policy for admins
az rest --method POST \
...
AZURE_KEYVAULT_LOGGING = RemediationTemplate(
    ...
    cli="""az monitor diagnostic-settings create \
  --name "kv-diagnostics" \
...
AZURE_SQL_AUDITING = RemediationTemplate(
    ...
    cli="""az sql server audit-policy update \
  --resource-group "{rg}" \
  --name "{resource}" \
  --state Enabled \
...
AZURE_SQL_ENTRA_ADMIN = RemediationTemplate(
    ...
    cli="""az sql server ad-admin create \
  --resource-group "{rg}" \
...
AZURE_SQL_DEFENDER = RemediationTemplate(
    ...
    cli="""az sql server advanced-threat-protection-setting update \
  --resource-group "{rg}" \
  --server "{resource}" \
  --state Enabled""",
...
AZURE_VM_JIT = RemediationTemplate(
    ...
    cli="""az security jit-policy create \
  --resource-group "{rg}" \
  --vm-name "{resource}" \
  --location "{region}" \
...
AZURE_AKS_RBAC = RemediationTemplate(
    ...
    cli="""# RBAC must be enabled at cluster creation time.
az aks create \
  --resource-group "{rg}" \
  --name "{resource}" \
  --enable-rbac \
...
AWS_ACCESS_ANALYZER = RemediationTemplate(
  ...
  --analyzer-name "security-analyzer-{region}" \
  --type ACCOUNT \
  --region "{region}" """,
...
AWS_S3_SSL = RemediationTemplate(
    ...
    cli="""aws s3api put-bucket-policy \
  --bucket "{resource}" \
...
AWS_SG_SSH = RemediationTemplate(
    ...
    cli="""# Remove the overly permissive rule
...
AWS_SG_RDP = RemediationTemplate(
    ...
    cli="""aws ec2 revoke-security-group-ingress --group-id "{resource}" --protocol tcp --port 3389 --cidr 0.0.0.0/0
...
AWS_LAMBDA_PUBLIC = RemediationTemplate(
    ...
    cli="""# Remove the public resource-based policy statement
...
AWS_LAMBDA_VPC = RemediationTemplate(
    ...
    cli="""aws lambda update-function-configuration \
...
OCI_IDENTITY_PASSWORD_POLICY = RemediationTemplate(
    ...
    cli="""oci iam authentication-policy update \
  --compartment-id "{tenancy_id}" \
...
OCI_NETWORK_SSH_SECLIST = RemediationTemplate(
    ...
    cli="""oci network security-list update \
...
OCI_NETWORK_SSH_NSG = RemediationTemplate(
    ...
    cli="""oci network nsg-rules update \
  --nsg-id "{resource_id}" \
...
OCI_KMS_ROTATION = RemediationTemplate(
    ...
    cli="""oci kms management key update \
  --key-id "{resource_id}" \
...
OCI_CLOUDGUARD = RemediationTemplate(
    ...
    cli="""oci cloud-guard configuration update \
  --compartment-id "{tenancy_id}" \
  --status ENABLED \
...
def get_remediation(check_id: str) -> Optional[RemediationTemplate]:
...
def render_remediation_block(template: RemediationTemplate, resource: str, **kwargs: str) -> str:
    ...
    return f"""## Remediation Playbook: {template.title}
