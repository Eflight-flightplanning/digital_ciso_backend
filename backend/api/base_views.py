...
class BaseRLSViewSet(BaseViewSet):
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Initialize DRF request to allow authentication
            ...
            tenant_id = (
                drf_request.auth.get("tenant_id")
                if drf_request.auth and hasattr(drf_request.auth, "get")
                else None
            )
            if tenant_id:
                drf_request.tenant_id = tenant_id
                cm = rls_transaction(tenant_id, using=self.db_alias)
            else:
                cm = nullcontext()
