...
DB_USER = settings.DATABASES["default"]["USER"] if not is_testing else "test"
DB_PASSWORD = (
    settings.DATABASES["default"]["PASSWORD"] if not is_testing else "test"
)
DB_PROWLER_USER = (
    settings.DATABASES["prowler_user"]["USER"] if not is_testing else "test"
)
DB_PROWLER_PASSWORD = (
    settings.DATABASES["prowler_user"]["PASSWORD"] if not is_testing else "test"
)
...
def delete_related_daily_task(provider_id: str):
    ...
    try:
        ...
        PeriodicTask.objects.using(MainRouter.admin_db).filter(name=task_name).delete()
    except Exception as e:
        logger.warning("Failed to delete periodic task for provider %s: %s", provider_id, e)
