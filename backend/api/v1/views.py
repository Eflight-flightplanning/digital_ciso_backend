...
from api.v1.serializers import (
    TenantLLMConfigSerializer,
    FindingAIAnalysisSerializer,
    RemediationPlaybookSerializer,
    CISOAdvisorConversationSerializer,

    ManagedServiceConfigSerializer,
...
class UserViewSet(BaseUserViewset):
    ...
    def create(self, request, *args, **kwargs):
        invitation_token = request.query_params.get("invitation_token", "").strip() or None
        if invitation_token in ("F3NMFPNDZHR4Z9", "string", "undefined", "null", ""):
...
class ManagedServiceConfigViewSet(BaseRLSViewSet):
    queryset = ManagedServiceConfig.objects.all()
    serializer_class = ManagedServiceConfigSerializer
    search_fields = ["operator_name", "operator_email"]
    ordering_fields = ["inserted_at", "updated_at"]
    ordering = ["-inserted_at"]


class HITLReviewQueueViewSet(BaseRLSViewSet):
    queryset = HITLReviewQueue.objects.select_related("finding", "scan", "assigned_analyst").all()
    serializer_class = HITLReviewQueueSerializer
    filterset_fields = ["status", "review_mode"]
    search_fields = ["finding__check_id", "notes"]
    ordering_fields = ["due_at", "inserted_at", "status"]
    ordering = ["due_at", "-inserted_at"]


class ReviewDecisionViewSet(BaseRLSViewSet):
    queryset = ReviewDecision.objects.select_related("review", "analyst").all()
    serializer_class = ReviewDecisionSerializer
    filterset_fields = ["decision"]
    search_fields = ["rationale", "internal_notes"]
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            tenant_id = self.request.tenant_id if hasattr(self.request, "tenant_id") else review.tenant_id
            DecisionLog.objects.create(
                tenant_id=tenant_id,
                review_id=review.id,
                finding_id=finding.id if finding else None,
                actor=self.request.user.email if hasattr(self.request.user, "email") else str(self.request.user),
                decision=decision.decision,
                reason=decision.rationale,
                action_taken=f"HITL Decision: {decision.decision}",
            )
        except Exception:
            pass


class DecisionLogViewSet(BaseRLSViewSet):
    queryset = DecisionLog.objects.all()
    serializer_class = DecisionLogSerializer
    filterset_fields = ["decision"]
    search_fields = ["actor", "reason", "action_taken"]
    ordering_fields = ["inserted_at"]
    ordering = ["-inserted_at"]



class TenantLLMConfigViewSet(BaseRLSViewSet):
    queryset = TenantLLMConfig.objects.all()
    serializer_class = TenantLLMConfigSerializer
...
class FindingAIAnalysisViewSet(BaseRLSViewSet):
    queryset = FindingAIAnalysis.objects.all()
    serializer_class = FindingAIAnalysisSerializer
    filterset_fields = ["domain", "exposure", "check_id"]
    search_fields = ["check_id", "summary", "root_cause", "attack_scenario"]
    ordering_fields = ["inserted_at", "confidence"]
    ordering = ["-inserted_at"]
...
class CISOAdvisorConversationViewSet(BaseRLSViewSet):
    queryset = CISOAdvisorConversation.objects.all()
    serializer_class = CISOAdvisorConversationSerializer
