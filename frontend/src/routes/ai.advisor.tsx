    ...
    <AppShell>
      <div className="h-[calc(100vh-5.5rem)] md:h-[calc(100vh-6.25rem)] flex flex-col gap-3 min-h-0 overflow-hidden">
        ...
        <div className="w-full flex-1 flex flex-col min-h-0 rounded-2xl border border-border/80 bg-surface/80 backdrop-blur-sm shadow-md overflow-hidden">
          ...
          <div className="px-4 py-2 border-b border-border/40 bg-surface/50 flex items-center gap-2 overflow-x-auto shrink-0 scrollbar-none">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5">
              {suggestedQueries.map((item, i) => (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-border/60 bg-surface-2/60 hover:bg-surface-2 px-2.5 py-1 text-[11px] text-muted-foreground hover:text-foreground hover:border-primary/40 transition-all shrink-0 cursor-pointer shadow-xs active:scale-95 whitespace-nowrap"
                  title={item.query}
                >
                  <span className="font-mono text-[9px] font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded border border-primary/20">
                    {item.tag}
                  </span>
                  <span className="max-w-[260px] sm:max-w-md truncate">{item.query}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-5 min-h-0">
              ...
              {/* Interactive Suggested Prompts Grid (shown on clean/initial stream) */}
              {messages.length <= 1 && !loading && (
                <div className="pt-2 pl-0 sm:pl-10.5 space-y-3 max-w-4xl animate-in fade-in duration-300">
                  <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    <Sparkles className="h-3.5 w-3.5 text-primary" />
                    <span>Recommended Threat Intelligence Queries ({providerFilter})</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {suggestedQueries.map((item, i) => (
                      <button
                        key={i}
                        onClick={() => handleSend(item.query)}
                        className="group flex flex-col justify-between text-left p-3.5 rounded-xl border border-border/80 bg-surface-2/40 hover:bg-surface-2 hover:border-primary/50 transition-all duration-200 shadow-xs hover:shadow-md cursor-pointer active:scale-[0.98]"
                      >
                        <div className="space-y-1.5">
                          ...
                          <p className="text-xs text-foreground/90 font-medium leading-snug pt-1">
                            {item.query}
                          </p>
                        </div>
                        <span className="text-[10px] text-muted-foreground group-hover:text-primary transition-colors pt-2 font-mono flex items-center gap-1">
                          Run analysis →
                        </span>
                      </button>
                    ))}
